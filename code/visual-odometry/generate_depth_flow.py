...
def readFlow(fn):
	...
	# Code adapted from:
	# http://stackoverflow.com/questions/2801ht0/reading-middlebury-flow-files-with-python-bytes-array-numpy
...
def get_depth_flow_and_3dflow(depth_path,flow_path,shape):
	...
	if(flow_path.endswith(".flo")):
		flow_=readFlow(flow_path)
		flow=flow_.reshape((-1,ht,wd))
	elif(flow_path.endswith(".png")or flow_path.endswith(".jpg")):
		flow_=imread(flow_path)
		flow=np.resize(flow_,(2,ht,wd))

	modified_index=np.ndarray.astype(index_depth+flow,int)
	bool_index=modified_index[:]>=ht
	modified_index[bool_index]=319
	bool_index=modified_index[:]>=wd
	modified_index[bool_index]=639
	...
	for i in range(r):        
		for j in range(c):
			x,y=row[i][j],col[i][j]
			new_depth_t1.append(depth_t1[[x],[y]])
